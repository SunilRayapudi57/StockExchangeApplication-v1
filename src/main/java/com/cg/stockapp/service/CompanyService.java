...
public class CompanyService implements ICompanyService {
	...
	public boolean addCompany(Company company) {
		...
		if(!managerRepo.existsById(managerId)) {
			log.warn("ManagerNotFoundException : Manager not found with id "+managerId);
			throw new ManagerNotFoundException("Creation", "Manager not found with id "+managerId);
		}
		Manager manager = managerRepo.findById(managerId).get();
		manager.setCompany(company);
		managerRepo.save(manager);
		repo.save(company);
		log.info("A new Company has been added successfully");
		return true;
	}
